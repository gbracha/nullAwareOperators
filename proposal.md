...
* Allows for 'elvis closurization' as in `a?.m` where *m* is a method, not because this is desired, but because it is not easily distinguished from other cases. In some sense this runs counter to the desire to clean up closurization as described in the [generalizedTearOffs]:[generalized tear offs proposal].  I'd rather not introduce things like `a?#m`.

[generalizedTearOffs]:https://github.com/gbracha/generalizedTearOffs
